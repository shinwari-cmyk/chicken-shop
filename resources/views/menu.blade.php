...
<div class="mb-4 text-center">
    ...
    @foreach($categories as $category)
        <a href="{{ route('menu', ['category' => $category->id]) }}"
           class="btn btn-outline-dark btn-sm me-2">{{ $category->name }}</a>
    @endforeach
</div>
        ...
        <div class="col-lg-4 col-md-6">
            <div class="card card-modern shadow-sm h-100">
                ...
                <div class="card-body text-center">
                    ...
                    <p class="fw-semibold mb-3 text-danger">Rs {{ $product->price }} / KG</p>
                    ...
                    <button class="btn btn-accent w-100 mb-2 add-cart" data-id="{{ $product->id }}">
                        🛒 Order via Website
                    </button>

                    <button class="btn btn-outline-dark w-100 whatsapp-btn" data-id="{{ $product->id }}">
                        📲 Order via WhatsApp
                    </button>
                </div>
            </div>
...
<!-- Toast -->
<div id="toastMsg" class="position-fixed top-0 end-0 m-4 p-3 rounded shadow"
...
document.querySelectorAll('.add-cart').forEach(btn=>{
    btn.addEventListener('click', function(){
        ...
        const weight=parseFloat(document.querySelector(`input[data-id="${id}"]`).value) || 0.5;
        axios.post("{{ url('cart/add') }}/"+id,{weight}).then(res=>{
            showToast();
